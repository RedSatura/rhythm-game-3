...
func spin_icon(status: bool) -> void:
	if status:
		...
		tween.tween_property(self, "rotation_degrees", 180, 0.5).set_trans(Tween.TRANS_ELASTIC)
		SignalHandler.emit_signal("toggle_show_title_screen_options", true)
	else:
		var tween: Tween = get_tree().create_tween()
		tween.tween_property(self, "rotation_degrees", 0, 0.5).set_trans(Tween.TRANS_ELASTIC)
